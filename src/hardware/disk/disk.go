...
type Disk struct {
	Name       string `json:"name"` // /sys/block/sda stored as /dev/sda
	RealPath   string `json:"path,"`
	Removable  bool   `json:"removable"` // /sys/block/sda/removable
	SectorSize uint64 `json:"-"`         // /sys/block/sda/queue/hw_sector_size
	Size       uint64 `json:"sizeKiB"`   // /sys/block/sda/queue/hw_sector_size * /sys/block/sda/size
}
	...
	err := filepath.Walk(blockdev_loc, func(path string, info fs.FileInfo, err error) error {
		...
		if l_path[len(l_path)-1] != info.Name() {
			...
			if _, e := os.Stat(devname); e == nil {
				...
				if !strings.HasPrefix(realpath, "/sys/devices/virtual") {
					disk := &Disk{
						Name:     devname,
						RealPath: realpath,
					}
